...
class TreeLSTM(torch.nn.Module):
    ...
    def __init__(self, in_features, out_features, hidden_units):
        ...
        self.U_iou = torch.nn.Linear((self.hidden_units // 16) * self.out_features,
                                     3 * (self.hidden_units // 16) * self.out_features, bias=False)
        ...
        self.U_f = torch.nn.Linear((self.hidden_units // 16) * self.out_features,
                                   (self.hidden_units // 16) * self.out_features, bias=False)

        self.feedforward = torch.nn.Linear((self.hidden_units // 16) * self.out_features, self.out_features, bias=False)
        # self.hidden_s = torch.nn.Linear((self.hidden_units // 16) * self.out_features, 256, bias=False)
        # self.hidden_s1 = torch.nn.Linear(256, 1000, bias=False)
        # self.hidden_s2 = torch.nn.Linear(1000, 128, bias=False)
        self.feedforward_s = torch.nn.Linear((self.hidden_units // 16) * self.out_features, 4, bias=False)
    ...
    def forward(self, features, node_order, adjacency_list, edge_order, root_node, root_label):
        ...
        # h_stance = self.hidden_s(h)
        # h_stance = self.hidden_s1(h_stance)
        # h_stance = self.hidden_s2(h_stance)
        h_stance = self.feedforward_s(h)
